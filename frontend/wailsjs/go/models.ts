export namespace admin {
	...
	export class DeleteAssetsResult {
	    deleted: {[key: string]: string};
	    deleted_counts: {[key: string]: any};
	    partial: boolean;
	    next_cursor?: string;
	    error?: api.ErrorResp;
	
	    static createFrom(source: any = {}) {
	        return new DeleteAssetsResult(source);
	    }
	
	    constructor(source: any = {}) {
	        if ('string' === typeof source) source = JSON.parse(source);
	        this.deleted = source["deleted"];
	        this.deleted_counts = source["deleted_counts"];
	        this.partial = source["partial"];
	        this.next_cursor = source["next_cursor"];
	        this.error = this.convertValues(source["error"], api.ErrorResp);
	    }
